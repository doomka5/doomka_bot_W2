...
async def _cancel_write_off_film_flow(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer("❌ Списание отменено.", reply_markup=WAREHOUSE_FILMS_KB)
...
def format_written_off_film_record(record: Dict[str, Any]) -> str:
    ...
    written_off_by_id_text = str(written_off_by_id) if written_off_by_id is not None else "—"
    return (
        f"{base_info}\n"
        f"Проект: {project}\n"
...
async def handle_write_off_warehouse_film(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Введите артикул пленки, чтобы списать её со склада.",
        reply_markup=CANCEL_KB,
    )
...
async def process_write_off_film_article(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Найдена запись:\n\n"
        f"{formatted}\n\n"
        "Укажите проект или назначение, на которое выполняется списание.",
        reply_markup=CANCEL_KB,
    )
