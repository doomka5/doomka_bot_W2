...
async def fetch_materials_with_details() -> list[dict[str, Any]]:
    if db_pool is None:
        raise RuntimeError("Database pool is not initialised")
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
...
def build_color_keyboard(colors: list[str]) -> ReplyKeyboardMarkup:
    rows: list[list[KeyboardButton]] = []
    for item in colors:
...
async def send_plastic_settings_overview(message: Message) -> None:
    materials = await fetch_materials_with_details()
    if materials:
        lines = []
...
async def handle_add_color_button(message: Message, state: FSMContext) -> None:
    ...
    if not materials:
        await message.answer(
            "Сначала добавьте материал, чтобы можно было указать цвета.",
            reply_markup=WAREHOUSE_SETTINGS_PLASTIC_KB,
        )
        await state.clear()
        return
...
async def process_add_color_material_selection(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Введите цвет (например, Белый, Красный, Синий).\n\n"
...
async def process_add_color_value(message: Message, state: FSMContext) -> None:
    ...
    if status == "material_not_found":
        ...
        await message.answer(
            "⚠️ Название цвета не может быть пустым. Попробуйте снова.",
            reply_markup=CANCEL_KB,
        )
        return
    else:
...
async def process_remove_color_material_selection(message: Message, state: FSMContext) -> None:
    if await _process_cancel_if_requested(message, state):
        return
    name = (message.text or "").strip()
    if not name:
    ...
    await message.answer(
        ...
        reply_markup=build_color_keyboard(match["colors"]),
    )
...
async def process_remove_color_value(message: Message, state: FSMContext) -> None:
    ...
    if not color:
        await message.answer(
            "⚠️ Укажите название цвета, который нужно удалить.",
            reply_markup=build_color_keyboard(await fetch_material_colors(material)),
        )
        return
