...
async def fetch_material_colors(material_name: str) -> list[str]:
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            WHERE LOWER(p.name) = LOWER($1)
            ORDER BY LOWER(c.color), c.color
            """,
            material_name,
        )
...
async def insert_material_color(material_name: str, color: str) -> str:
    ...
    return "added" if row else "error"
...
def normalize_color_input(raw_text: str) -> Optional[str]:
    if raw_text is None:
        return None
...
def build_colors_keyboard(colors: list[str]) -> ReplyKeyboardMarkup:
    rows: list[list[KeyboardButton]] = []
    for color in colors:
        rows.append([KeyboardButton(text=color)])
    rows.append([KeyboardButton(text="❌ Отмена")])
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True)
...
async def handle_add_color_button(message: Message, state: FSMContext) -> None:
    ...
    if not materials:
        await message.answer(
            "Сначала добавьте материал, чтобы указать доступные цвета.",
            reply_markup=WAREHOUSE_SETTINGS_PLASTIC_KB,
        )
        await state.clear()
        return
...
async def process_add_color_material_selection(message: Message, state: FSMContext) -> None:
    name = (message.text or "").strip()
    if not name:
        await message.answer("⚠️ Название не может быть пустым. Попробуйте снова.")
        return
    ...
    await message.answer(
        "Введите название цвета (например, Белый, Чёрный, Прозрачный).\n\n"
...
async def process_add_color_value(message: Message, state: FSMContext) -> None:
    ...
    if not material:
        ...
        return
    color = normalize_color_input(message.text)
    if color is None:
        await message.answer(
            "⚠️ Укажите название цвета, например Белый или Чёрный.",
            reply_markup=CANCEL_KB,
        )
        return
    ...
    if status == "material_not_found":
        ...
        await message.answer(
            "⚠️ Не удалось добавить цвет. Попробуйте снова позже.",
            reply_markup=WAREHOUSE_SETTINGS_PLASTIC_KB,
        )
    await state.clear()
    await send_plastic_settings_overview(message)
...
async def handle_remove_color_button(message: Message, state: FSMContext) -> None:
    ...
    if not materials_with_colors:
        await message.answer(
            "Пока нет материалов с цветами для удаления.",
            reply_markup=WAREHOUSE_SETTINGS_PLASTIC_KB,
        )
        await state.clear()
        return
...
async def process_remove_color_material_selection(
    ...
    if match is None:
        ...
        return
    await state.update_data(selected_material=match["name"], available_colors=match["colors"])
...
async def process_remove_color_value(message: Message, state: FSMContext) -> None:
    ...
    color = normalize_color_input(message.text)
    if color is None:
        available_colors = data.get("available_colors") or await fetch_material_colors(material)
        await message.answer(
            "⚠️ Укажите цвет из списка.",
            reply_markup=build_colors_keyboard(available_colors),
        )
        return
    ...
    if status == "material_not_found":
        ...
        refreshed_colors = await fetch_material_colors(material)
        await state.update_data(available_colors=refreshed_colors)
        await message.answer(
            f"ℹ️ Цвет «{color}» не найден у «{material}».",
            reply_markup=build_colors_keyboard(refreshed_colors),
        )
        return
    await state.clear()
    await send_plastic_settings_overview(message)
